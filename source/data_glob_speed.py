...
class SequenceToSequenceDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        pos = np.copy(self.gt_pos[seq_id][frame_id - self.window_size:frame_id])

        # if self.transform is not None:
        #     feat, targ = self.transform(feat, targ)

        return feat.astype(np.float32), targ.astype(np.float32), seq_id, frame_id, self.data_path[seq_id]
